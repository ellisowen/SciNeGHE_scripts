...
# Specify whether simulation or data analysis
...
if analysis == 'Simulation':
    ...
    source_image_true = fits.getdata('in_sources.fits')
    diffuse_image_true = fits.getdata('in_diffuse.fits')
    ...
    true_image_hdu = fits.open('sources.fits.gz')
    true_image_hdu[1].data = total_image_true
    #import IPython; IPython.embed()
    true_image_hdu.writeto('initial_image.fits', clobber=True)
    ...
    total_image_true = fits.getdata('in_counts_map.fits')
    counts = total_image_true
...
if __name__ == '__main__':
    ...
    ibe.run(n_iterations=NUMBER_OF_ITERATIONS, filebase='test')
